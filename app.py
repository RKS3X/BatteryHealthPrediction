...
with tab_prep:
    ...
    steps = [
        ...
        ("5. ปรับสเกลข้อมูล (Feature Scaling)",
         ...
         "แล้วนำไป transform ชุด Test ขั้นตอนนี้จำเป็นมากสำหรับโมเดลที่วัดระยะทางอย่าง KNN และ SVR เพราะฟีเจอร์ในข้อมูล "
...
with tab_theory:
    ...
    st.markdown(
        "เพื่อหาว่าอัลกอริทึมใดเหมาะกับการทำนาย Battery Health มากที่สุด จึงเทรนโมเดล Regression 5 แบบ "
    ...
    models_info = [
        ...
        ("Support Vector Regression (SVR) — RBF Kernel", "KERNEL-BASED · ตัวที่ใช้จริงในแอปนี้",
         "พยายามหาฟังก์ชันที่ทำนายค่าให้อยู่ภายในขอบเขตความคลาดเคลื่อนที่ยอมรับได้ (epsilon-tube) รอบแนวโน้มของข้อมูล "
         "จุดข้อมูลที่อยู่นอกขอบเขตนี้เท่านั้นที่จะมีผลต่อการปรับโมเดล (เรียกว่า Support Vectors) โปรเจกต์นี้ใช้ RBF Kernel "
         "ซึ่งแปลงข้อมูลไปอยู่ในมิติที่สูงขึ้นโดยปริยาย (kernel trick) ทำให้จับความสัมพันธ์แบบไม่เป็นเส้นตรง (non-linear) ระหว่าง"
         "ฟีเจอร์กับ Battery Health ได้ดี พารามิเตอร์ที่ปรับคือ C = 10 (ควบคุมความเข้มงวดต่อความคลาดเคลื่อน) และ epsilon = 0.5"),
    ]
...
with tab_eval:
    ...
    st.markdown(
        "ประเมินโมเดลทั้ง 5 แบบด้วยชุด Test (240 แถว) โดยใช้ 3 ตัวชี้วัดหลัก: **MAE** (ค่าคลาดเคลื่อนเฉลี่ยสัมบูรณ์ "
    ...
    g3, g4 = st.columns(2)
    with g3:
        st.image("assets/chart_actual_vs_pred.png", caption="ค่าจริงเทียบค่าที่ทำนายจากโมเดล SVR (RBF)", use_container_width=True)
    with g4:
    ...
    st.markdown(
        ...
        <div class="info-card">
        <h4>สรุปผล</h4>
        SVR (RBF) ให้ผลแม่นยำที่สุด (R² = 0.970) ตามมาด้วย Random Forest ที่ใกล้เคียงกันมาก (R² = 0.969)
        ทั้งสองแบบทำผลได้ดีกว่า Linear Regression ชัดเจน สะท้อนว่าความสัมพันธ์ระหว่างฟีเจอร์กับ Battery Health
        มีความไม่เป็นเส้นตรงอยู่บ้าง จากกราฟ Feature Importance พบว่า <b>Cycle Count</b> (จำนวนรอบชาร์จ) และ
        <b>Full Charge Capacity</b> (ความจุที่ชาร์จได้จริง) เป็นสองปัจจัยที่มีผลต่อการทำนายมากที่สุดอย่างชัดเจน
